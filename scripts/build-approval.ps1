...
$Source = Join-Path $Root "components\providers\approval-policy-provider"
$Classes = Join-Path $Root "build\approval-policy-provider-classes"
$Stage = Join-Path $Root "build\component-source\approval-policy-provider"
$ProviderJar = Join-Path $Root "build\approval-policy-provider.jar"
$Output = Join-Path $Root "build\approval-policy-provider-1.0.0.carbon"
...
if ($LASTEXITCODE -ne 0) { throw "Approval Policy Provider compilation failed." }
& jar --create --file $ProviderJar --main-class io.carbongate.provider.approval.ApprovalPolicyProvider `
    -C $Classes . -C (Join-Path $Root "build\classes") "io/carbongate/json"
if ($LASTEXITCODE -ne 0) { throw "Approval Policy Provider JAR creation failed." }
...
if ($LASTEXITCODE -ne 0) { throw "Approval Policy Provider package build failed." }
Write-Host "Built Approval Policy Provider $Output"
