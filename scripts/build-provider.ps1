...
$Source = Join-Path $Root "components\providers\sensitive-data-provider"
$Classes = Join-Path $Root "build\sensitive-data-provider-classes"
$Stage = Join-Path $Root "build\component-source\sensitive-data-provider"
$ProviderJar = Join-Path $Root "build\sensitive-data-provider.jar"
$Output = Join-Path $Root "build\sensitive-data-provider-1.0.0.carbon"
...
if ($LASTEXITCODE -ne 0) { throw "Sensitive Data Provider compilation failed." }
& jar --create --file $ProviderJar --main-class io.carbongate.provider.dlp.SensitiveDataProvider `
    -C $Classes . -C (Join-Path $Root "build\classes") "io/carbongate/json"
if ($LASTEXITCODE -ne 0) { throw "Sensitive Data Provider JAR creation failed." }
...
if ($LASTEXITCODE -ne 0) { throw "Sensitive Data Provider package build failed." }
Write-Host "Built Sensitive Data Provider $Output"
