...
ASSET_NAME=$("$ROOT/scripts/release-asset-name.sh" "native.$PLATFORM.asset" "$VERSION")
case "$ASSET_NAME" in
  *.tar.gz) NAME=${ASSET_NAME%.tar.gz} ;;
  *.zip) NAME=${ASSET_NAME%.zip} ;;
  *) printf 'Unsupported native archive type: %s\n' "$ASSET_NAME" >&2; exit 1 ;;
esac
STAGE="$ROOT/build/release/$NAME"
ASSETS="$ROOT/build/release-assets"
rm -rf "$STAGE"
mkdir -p "$STAGE/bin" "$STAGE/config" "$STAGE/distribution" "$STAGE/docs" "$STAGE/licenses" \
...
case "$PLATFORM" in
  windows-*)
    ...
    (cd "$ROOT/build/release" && jar --create --file "$ASSETS/$ASSET_NAME" --no-manifest "$NAME")
    ASSET="$ASSETS/$ASSET_NAME"
    ;;
  *)
    cp "$ROOT/scripts/install-native.sh" "$STAGE/install.sh"
    chmod +x "$STAGE/install.sh" "$STAGE/bin/carbon"
    tar -C "$ROOT/build/release" -czf "$ASSETS/$ASSET_NAME" "$NAME"
    ASSET="$ASSETS/$ASSET_NAME"
    ;;
esac
